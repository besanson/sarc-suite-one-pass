...
CH5 (compensation is empirically unsafe, discrete grid). Over the pre-registered weight/threshold grid (66 weight vectors x 4 thresholds, 264 cells), does any cell's weighted-score aggregator admit at least one of the 98 min-join-Held profiles out of the full 125-point three-gate verdict space, while the min join admits zero of them by construction. Result: proposition_1_holds_exhaustively = True. Two distinct quantities are reported here, with distinct labels and distinct denominators, since they are not the same measurement: the **formal result** is the maximum number of compensated response profiles at a single grid cell, 48 out of 98 Held response profiles, from the exhaustive discrete checker (`checkers/compensation_check.py`); the **empirical result** is the mean number of compensated held decision instances pooled across scenarios S1-S4, over the 30-seed sweep: 13775.1 (95% CI [13753.5, 13796.6], n=30) (`ch5_aggregator.py`, `out/results/sweep_summary.json`). The two are not comparable by denominator (one counts formal response profiles out of the full discrete grid's Held population; the other counts individual decision instances actually produced across four scenarios' worth of simulated traffic), and this paper does not conflate them under one name.
...
Six citation gaps were resolved via the V4 pipeline (fetch, verify, record in verified-citations.json) for this response to the independent review's finding F5; none were invented, and any that could not have been verified would have stayed an honestly unresolved placeholder, counted rather than guessed. This section's expansion, following a commissioned second (positioning) review, adds further gaps the same pipeline has not yet resolved; each is marked [CITE-NEEDED] rather than filled with an unverified reference, per this artifact's own citation-gate discipline, and is left for the founder's human literature pass.

Compositional runtime enforcement. Traditional runtime-enforcement work commonly reasons over event traces and enforcement policies, asking whether independently correct monitors remain correct under serial or parallel composition, and under what conditions enforcement interference is avoided [CITE-NEEDED: compositional runtime enforcement monitor correctness]. This paper studies a different object: heterogeneous pre-action controls where a remediation changes the semantic object another control subsequently evaluates, not merely whether two trace-level monitors compose.
...
Policy-combining algorithms. "One veto dominates other permits" is already a familiar policy-composition pattern, for example deny-overrides-style combining algorithms [CITE-NEEDED: policy-combining algorithms, deny-overrides]. This paper therefore does not present the join rule itself as its principal novelty; the difficult composition problem it studies begins when a control's response includes a transformation, not merely a decision label.
...
Pre-action governance cannot be treated as a bag of independent checks once controls are permitted to transform the proposed action or update shared governance state. Correct composition must distinguish hard feasibility from preference aggregation, reevaluate dependent controls after remediation, define remediation order when operators do not commute, and preserve the provenance of the action and evidence actually executed. General termination, confluence, and concurrency for arbitrary stateful remediation systems remain open.
...
Version 0.4 revises framing, positioning, and related-work coverage following a commissioned second review (review-secondary/), with all measured results, claim semantics, PROOF-STATUS tags, and the four-round review chain unchanged.
